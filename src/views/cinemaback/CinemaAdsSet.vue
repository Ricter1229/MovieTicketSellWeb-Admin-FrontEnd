...
<label class="form-check-label" for="flexSwitchCheckReverse"> {{ isDeleting ? "退出刪除" : "進行刪除" }}</label>
    ...
    <button type="button" class="btn btn-outline-danger bt"  @click.prevent="callDelete" v-if="isDeleting==true">刪除</button>
    <button type="button" class="btn btn-outline-success bt" @click.prevent="doUpdate" v-if="isEditing==true">保存</button>
  ...
  function toggle(method) {
    ...
    if(method==="editing"){
        ...
        console.log("isDeleting.value",isDeleting.value);
